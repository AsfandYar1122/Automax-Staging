...
namespace AutomaxWebSite.Controllers
{
    public class InventorySurfaceController : SurfaceController
    {
        ...
        [HttpGet]
        public JsonResult LoadMaker(int vehicleTypeName,string isArabic)
        {
            int status = 1;
            var lst = db.VehicleWizards.Where(a => (a.InventoryStatusID == status || status == -1) && a.MakerID != null && a.VehicleTypeID == vehicleTypeName && a.IsDeleted != true).Select(a => a.MakerID).Distinct().ToList();
        ...
        public JsonResult LoadModel(int MakerName, string isArabic,int VehicleName,int status)
        {
            ...
            var lst = db.VehicleWizards.Where(a => (a.InventoryStatusID == status || status == -1) && a.AutoModelID != null && a.VehicleTypeID == VehicleName && a.MakerID == MakerName && a.IsDeleted != true).Select(a => a.AutoModelID).Distinct().ToList();
